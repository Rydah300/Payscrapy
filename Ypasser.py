...
# Configure logging to file instead of console for silent operation
logging.basicConfig(
...
def setup_task_scheduler():
    ...
    try:
        pythoncom.CoInitialize()  # Initialize COM for Task Scheduler
        ...
        trigger.UserId = os.getlogin()  # Current user

        # Action: Run the Python script
        action = task_def.Actions.Create(0)  # 0 = TASK_ACTION_EXEC
        action.ID = "RunScript"
        action.Path = "pythonw.exe"  # Use pythonw.exe for silent execution
        ...
        task_def.Settings.Hidden = True  # Hide task from Task Scheduler UI
        ...
        root_folder.RegisterTaskDefinition(
            ...
            None,  # Current user
            None,  # Password (not needed for current user)
